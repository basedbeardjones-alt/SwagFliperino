...
public class SuggestionController {
    ...
    // If true, the plugin will NOT require Copilot login.
    ...
    public void getSuggestionAsync() {
        ...
        // Always require a valid OSRS login state (you're actually in-game),
        // but only require Copilot login if SKIP_AUTH is NOT enabled.
        if (!osrsLoginManager.isValidLoginState()) {
            return;
        }
        if (!SKIP_AUTH && !copilotLoginManager.isLoggedIn()) {
            return;
        }
        ...
        Consumer<HttpResponseException> onFailure = (e) -> {
            ...
            // Only force Copilot logout UI if we are NOT skipping auth.
            if (e.getResponseCode() == 401 && !SKIP_AUTH) {
        ...
        apiRequestHandler.getSuggestionAsync(
                accountStatus.toJson(
                        gson,
                        grandExchange.isOpen(),
                        config.priceGraphWebsite() == FlippingCopilotConfig.PriceGraphWebsite.FLIPPING_COPILOT
                ),
                suggestionConsumer,
                graphDataConsumer,
                onFailure
